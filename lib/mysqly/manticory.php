...
class manticory
{
  ...
  public static function exec($sql, $bind = [])
  {
    if (!static::$db) {
      ...
      $dsn = 'mysql:host=' . (static::$auth['host'] ?? '127.0.0.1') .
        ';port=' . (static::$auth['port'] ?? '9306') .
        (!empty(static::$auth['db']) ? ';dbname=' . static::$auth['db'] : '');

      $user = static::$auth['user'] ?? null;
      $pwd = static::$auth['pwd'] ?? null;
      static::$db = $user === null ? new PDO($dsn) : new PDO($dsn, $user, $pwd);
  ...
  public static function auth($user = null, $pwd = null, $db = null, $host = '127.0.0.1', $port = 9306)
  {
