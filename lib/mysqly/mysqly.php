...
class mysqly
{
  ...
  protected static function connect()
  {
    ...
    static::$db = new PDO(
      'mysql:host=' . (static::$auth['host'] ?? '127.0.0.1') .
        ';port=' . (static::$auth['port'] ?? '3306') .
        (!empty(static::$auth['db']) ? ';dbname=' . static::$auth['db'] : '') .
        ';charset=utf8mb4',
      static::$auth['user'],
      static::$auth['pwd'],
      [
    ...
    for ($attempt = 0; $attempt < 2; $attempt++) {
      try {
        ...
        foreach ($params as $k => $v) {
          ...
          elseif (is_null($v))   $type = PDO::PARAM_NULL;
        ...
        // 🔥 МЕРТВИЙ КОНЕКТ → reconnect + retry
        if (
          $attempt === 0 &&
          (
  ...
  public static function auth($user, $pwd, $db, $host = 'localhost', $port = 3306)
  {
    static::$auth = ['user' => $user, 'pwd' => $pwd, 'db' => $db, 'host' => $host, 'port' => $port];
  }
  ...
  {
    return static::insert($table, $data, false, true);
  }
  ...
  public static function multi_insert($table, $rows, $ignore = false, $replace = false)
  {
    ...
    foreach ($rows as $r => $row) {
      $values[] = '(' . implode(',', array_map(function ($c) use ($r) {
        return ":r{$r}_{$c}";
      }, range(0, count($row) - 1))) . ')';
  ...
  {
    return static::multi_insert($table, $rows, false, true);
  }
